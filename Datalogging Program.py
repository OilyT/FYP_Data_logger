            ...
            while self.running:
                with open(self.output_file_path, "a") as file:
                    if self.datum_active.get():
                        ...
                        self.torque = sf.calculate_torque(self.data, float(self.text_vars[4].get()),
                                                          float(self.text_vars[5].get()),
                                                          float(self.text_vars[6].get()))
                        self.speed = sf.calculate_speed(self.data)
                    if self.vsd_active.get():
                        self.pump_rpm = sf.read_arduino(self.ser2)
                    current_time = time.time() - self.start_time
                    current_time = round(current_time, 2)
                    file.write(str(current_time) + "," + str(self.torque) + "," + str(self.speed) + "," + str(
                        self.pump_rpm) + "\n")
                time.sleep(0.05)
        except Exception as e:
            if not self.running:
...
class GUIApp:
    def __init__(self, root):
        ...
        for i, label_text in enumerate(
                ["Datum COM Port", "Datum Baud Rate", "VSD COM Port", "VSD Baud Rate", "Calibration Figure(mV/V)",
                 "Torque Offset", "Torque Range"]):
    ...
    def toggle_thread(self):
        if not self.logging_thread or not self.logging_thread.is_alive():
            self.logging_thread = LoggingThread(self.output_file_path, self.text_vars, self.datum_active,
                                                self.vsd_active, self.error_label)
